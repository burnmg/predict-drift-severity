 ...
 * @version $Revision: 7 $
 */
public class DriftingSEAGenerator extends AbstractOptionHandler implements
        DriftingStream {
    ...
    public IntOption noisePercentageOption = new IntOption("noisePercentage",
            'p', "Percentage of noise to add to the data.", 5, 0, 100);
	...
	{
		int nextFunction = instanceRandom.nextInt(3) + 1;
		while(functionOption.getValue()==nextFunction)
		{
			nextFunction = instanceRandom.nextInt(3) + 1;
		}
		functionOption.setValue(nextFunction);
		restart();
	}
}
