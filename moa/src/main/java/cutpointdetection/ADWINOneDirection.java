...
public class ADWINOneDirection implements CutPointDetector {
			...
			while (blnReduceWidth) // Diference
			{
				...
				do {
					for (int k = 0; k <= (cursor.bucketSizeRow - 1); k++) {
						...
						u2 = cursor.Total(k);
						if (n0 > 0) {
							v0 += cursor.Variance(k) + n0 * n2 * (u0 / n0 - u2 / n2) * (u0 / n0 - u2 / n2) / (n0 + n2);
						}
						if (n1 > 0) {
							v1 -= cursor.Variance(k) + n1 * n2 * (u1 / n1 - u2 / n2) * (u1 / n1 - u2 / n2) / (n1 + n2);
						}
	...
	private boolean blnCutexpression(int n0, int n1, double u0, double u1, double v0, double v1, double absvalue,
		...
		// pWeight = (Math.abs(absvalue) - epsilon + 1) > pWeight ?
