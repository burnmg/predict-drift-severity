 ...
 * </p>
 * 
 * @author Paulo Gon�alves (paulomgj at gmail dot com) Modified by David Huang
 *
 */
public class PHT implements CutPointDetector
{
	...
	public void setMagnitudeThreshold (double t)
	{
		this.magnitudeThreshold = t;
	}
	...
	{
		/*
		 * if (prediction == false) { p = p + (1.0 - p) / nt; } else { p = p -
		 * (p) / nt; }
		 */
		p = value;
		nt++;
		...
		/*
		 * else if(nt > PHT_MINNUMINST && PHt > warningThreshold) { // return
		 * DriftDetectionMethod.DDM_WARNING_LEVEL; } else { //return
		 * DriftDetectionMethod.DDM_INCONTROL_LEVEL; }
		 */ return false;
	}
